  ...
  update(scroll) {
    ...
    if (offsetBottom >= this.offset.top) {
      ...
      if (this.direction === 'left') {
        ...
        this.element.style[this.transform] = `translate3d(${this.parallax}px, 0, 0)`;
      } else if (this.direction === 'right') {
        this.parallax = Math.max(this.parallax, 0);
        this.element.style[this.transform] = `translate3d(${this.parallax}px, 0, 0)`;
      } else {
        // Default to vertical movement if direction is not specified or is 'up'/'down'
        this.element.style[this.transform] = `translate3d(0, ${this.parallax}px, 0)`;
      }
    } else {
      if (this.direction === 'left' || this.direction === 'right') {
        this.element.style[this.transform] = `translate3d(${this.amount}px, 0, 0)`;
      } else {
